<template>
	<div class="index">
		<div class="navs">
			<ul>
				<li v-for="(item, index) in nav" :key="index" :class="navIndex==index?'active':''" @click="ADcolor(index)">{{item}}</li>
			</ul>
		</div>
		<div class="container">
			<div class="row flex" style="margin-top: 30px;" v-for="(item, index) in arrItem" :key="'c' + index + item.name">
				<div class="left">
					<!-- Swiper -->
					<div class="gallery-top-c">
						<img :src="item.imgs[item.showIndex]['src']" v-if="item.imgs.length" />
					</div>
					<div class="gallery-thumbs" v-if="item.imgs.length">
						<div class="gallery-pre" @click="handleSwiper(item, 'pre')"></div>
						<div class="gallery-con">
							<div class="imgs" :data-id="item.name">
								<img :src="img.src" v-for="(img, index) in item.imgs" :class="index == item.showIndex ? 'activeImg' : ''" />
							</div>
						</div>
						<div class="gallery-next" @click="handleSwiper(item, 'next')"></div>
					</div>
				</div>
				<div class="center">
					<div><i>{{item.name}}</i><span class="span" v-if="item.leave">{{item.leave}}景区</span></div>
					<div>景区地址：{{item.adress}}</div>
					<div>开放时间：<span class="p">{{item.openTime}}</span></div>
					<div class="btn" @click="$router.push({name: '知四方详情', params: {id: item.name}})">点击查看路线免费语音讲解攻略</div>
				</div>
				<div class="right">
					{{item.info}}
				</div>
		...
		watch: {
			navIndex(index){
				if(index == 0){
					...
					this.arrItem = this.xiangcun
				}else if(index == 2){
					this.arrItem = this.jingqu
				}else if(index == 3){
					this.arrItem = this.bowu
				}
			}
		},
		methods: {
			...
			handleSwiper(item, t){
				if(t == 'next'){
					if(item.showIndex === item.imgs.length - 1){
						return
					}
					item.showIndex++
					...
					if((item.showIndex) % 3 == 0){
						
						document.querySelectorAll('.imgs').forEach(a => {
							if(a.dataset.id == item.name){
								a.style['margin-left'] = -((item.showIndex) / 3 * 3 * 92) + 'px'
							}
						})
					...
					if((item.showIndex + 1) % 3 == 0){
						document.querySelectorAll('.imgs').forEach(a => {
							if(a.dataset.id == item.name){
								a.style['margin-left'] = -((item.showIndex - 2) / 3 * 3 * 92) + 'px'
							}
						})
					}
...
<style scoped lang="less">
	...
	.gallery-thumbs {
		...
		.gallery-pre{
			width: 10px;
			height: 17px;
			background: url(../../assets/img/index/zz.png) no-repeat center center;
			background-size: 100% 100%;
		}
		.gallery-next{
			width: 10px;
			height: 17px;
			background: url(../../assets/img/index/yy.png) no-repeat center center;
			background-size: 100% 100%;
		}
	}
	.gallery-con{
	...
	.navs {
		...
		box-shadow: 0px 0px 24px 0px rgba(48, 48, 48, 0.17);
		ul {
			width: 1173px;
			margin: 0 auto;
			overflow: hidden;
		...
		.active {
			background: url(../../assets/img/index/select.png) no-repeat left;
		}
	}
